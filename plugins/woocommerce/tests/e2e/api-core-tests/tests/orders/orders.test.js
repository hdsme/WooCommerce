		...
		it( 'dp (precision)', async () => {
			...
			const verifyOrderPrecision = ( order, dp ) => {
				...
				order[ 'line_items' ].forEach( ( lineItem ) => {
					...
					// Verify that price is a Number? (and not affected by dp?)

					// There is a potential bug with the taxes formatting, disabled for now.
					// lineItem[ 'taxes' ].forEach( ( tax ) => {
					// 	expectPrecisionToMatch( tax[ 'total' ], dp );
					// 	expectPrecisionToMatch( tax[ 'subtotal' ], dp );
					// } );
				} );
				...
				order[ 'shipping_lines' ].forEach( ( shippingLine ) => {
					...
					// There is a potential bug with the taxes formatting, disabled for now.
					// shippingLine[ 'taxes' ].forEach( ( tax ) => {
					// 	expectPrecisionToMatch( tax[ 'total' ], dp );
					// 	expectPrecisionToMatch( tax[ 'subtotal' ], dp );
					// } );
				} );
				...
				order[ 'fee_lines' ].forEach( ( feeLine ) => {
					...
					// There is a potential bug with the taxes formatting, disabled for now.
					// feeLine[ 'taxes' ].forEach( ( tax ) => {
					// 	expectPrecisionToMatch( tax[ 'total' ], dp );
					// 	expectPrecisionToMatch( tax[ 'subtotal' ], dp );
					// } );
				} );

				// Verify that discounts are not affected by dp?
